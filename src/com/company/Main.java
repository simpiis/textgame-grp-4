        ...
        while(round <= 49){
            ...
            if(rooms[position].getMonster()!= null){
                if ( rooms[position].getMonster() == minotaur) {
                    ...
                    position = combatMethod(hero, typhone, playerInventory, score, keyboard, position);
                    if(positionCombat == position && typhone.getHp() <=0) {
                        rooms[position].setMonster(null);
                    }
                } else {
                    int positionPrev = position;
                    int positionCombat = position;
                    if (positionPrev == position +7){
                        round-=1;
                    }
                    position=combatMethod(hero, wampa, playerInventory, score, keyboard, position);
